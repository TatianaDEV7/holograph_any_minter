    ...
    def mint(self):
        ...
        while True:
            try:
                ...
                if status == 1:
                    logger.info(
                        f'{self.address}:{self.chain} - успешно заминтил {self.count} FLWR {scans[self.chain]}{self.w3.to_hex(hash_)}...')
    ...
    def check_nft(self):
        if self.mode == 0 and self.chain != 'opti':
            ...
            try:
                ...
                if id_:
                    logger.success(f'{self.address} - FLWR {id_} nft founded on {self.chain}...')
                    return id_
            except Exception as e:
                logger.error(f'{self.address} - FLWR nft not in wallet...')
                return False
            ...
            for chain in ['avalanche', 'polygon', 'bsc']:
                ...
                try:
                    ...
                    id_ = int(result['result'][0]['token_id'])
                    if id_:
                        logger.success(f'{self.address} - FLWR {id_} nft founded on {chain}...')
    ...
    def bridge(self):
        if self.mode == 0:
            ...
            if data:
                ...
                return self.address, 'FLWR nft not in wallet'
        ...
        while True:
            ...
            try:
                ...
                if status == 1:
                    logger.success(
                        f'{self.address}:{self.chain} - successfully bridged FLWR {nft_id} to {self.to} : {self.scan}{self.w3.to_hex(hash_)}...')
                    self.sleep_indicator(self.delay)
                    return self.address, 'success'
            except Exception as e:
                error = str(e)
